...
class VaultRestControllerTest {
    ...
    @Mock
    private VaultService vs;
    @Mock
    private AuthUtil au;

    @BeforeEach
    void setUp() {
        mockMvc = MockMvcBuilders.standaloneSetup(new VaultRestController(vs, au)).build();
        User u = new User();
        u.setId(1L);
        when(au.getCurrentUser()).thenReturn(u);
    }
    ...
    void testAddEntry() throws Exception {
        mockMvc.perform(post("/api/vault")
                ...
                .content("{\"accountName\":\"T\",\"password\":\"p\"}"))
